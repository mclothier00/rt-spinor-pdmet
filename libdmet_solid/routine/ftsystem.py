...
def fermi_smearing_occ(mu, mo_energy, beta):
    ...
    mu = np.asarray(mu).reshape(-1, *np.ones(mo_energy.ndim - 1, dtype=np.int))
...
def find_mu_by_density(density, mo_energy, beta, mu0=None, \
    ...
    norb = mo_energy.size
    nelec = density * norb
    return find_mu(nelec, mo_energy, beta, mu0=mu0, f_occ=f_occ, \
        tol=tol*norb)
...
def get_dw_dv(mo_energy, mo_coeff, drho, mu, beta, \
        fix_mu=True, compact=False, fit_idx=None):
    """
    ...
    for s in range(spin):
        ...
        dw_dv[s] = mdot(mo_coeff[s, fit_idx].T, 2.0*drho[s], \
                mo_coeff[s, fit_idx].conj()) * K 
    ...
    if not fix_mu:
        ...
        for s in range(spin):
            ...
            if abs(f_sum) > ZERO_TOL: # not almost zero T
                # partial rho_ij / partial mu
                drho_dmu = (mo_coeff[s] \
                        * f).dot(mo_coeff[s].conj().T) # should * beta
