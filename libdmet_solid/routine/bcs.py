...
from libdmet_solid.routine.bcs_helper import *
from libdmet_solid.routine.slater import MatSqrt, orthonormalizeBasis
from libdmet_solid.routine.mfd import assignocc, HFB
    ...
    if lattice.is_model:
        ...
        if local:
            ...
            log.warn("Nonlocal basis is in test and only models \
                    are supported.")
            from libdmet_solid.integral.integral_nonlocal_emb import transform
...
def FitVcorEmb(GRho, lattice, basis, vcor, mu, beta=np.inf, \
    ...
    if kwargs.get("test_grad", False):
        from libdmet_solid.routine.slater import test_grad
...
def transformResults(GRhoEmb, E, lattice, basis, ImpHam, \
    ...
    if E is not None:
        ...
        from libdmet_solid.dmet.HubbardBCS import apply_dmu
