...
from libdmet_solid import settings
from libdmet_solid.routine import localizer
from libdmet_solid.routine import ftsystem
from libdmet_solid.routine import mfd
from libdmet_solid.routine.fit import minimize
from libdmet_solid.routine import pbc_helper as pbc_hp
from libdmet_solid.solver.scf import _get_jk, _get_veff, restore_Ham
from libdmet_solid.basis_transform import make_basis
from libdmet_solid.basis_transform.eri_transform import get_emb_eri, get_unit_eri
from libdmet_solid.lo.lowdin import vec_lowdin
from libdmet_solid.routine.slater_helper import *
from libdmet_solid.utils.misc import *
...
def get_emb_Ham(lattice, basis, vcor, local=True, **kwargs):
    ...
    return integral.Integral(nbasis, spin == 1, False, H0,
            {"cd": H1}, {"ccdd": H2}, ovlp=ovlp_emb), None
    ...
    if lattice.is_model:
        ...
        if int_bath: # interacting bath
            H2 = get_emb_eri(cell, mydf, C_ao_lo=C_ao_lo, basis=basis, \
                    kscaled_center=kscaled_center, symmetry=eri_symmetry, \
                    max_memory=max_memory, swap_idx=swap_idx, \
                    t_reversal_symm=t_reversal_symm)
            if last_aabb and H2.shape[0] == 3:
                H2 = H2[[0, 2, 1]]
...
def foldRho(rho, lattice, basis):
    ...
    for s in range(spin):
        # ZHC FIXME seems without sparse is faster
        #rdm1_emb[s] = transform_trans_inv_sparse(basis[s], lattice, rho[s])
...
def get_H1_scaled(H1, imp_idx, env_idx=None):
    ...
    nbasis = H1.shape[-1]
    if env_idx is None:
        env_idx = np.asarray([idx for idx in range(nbasis) if idx not in imp_idx])
    ...
    print("imp_env", imp_env)
    print("env_imp", env_imp)
    print("env_env", env_env)
...
def transformResults(rhoEmb, E, basis, ImpHam, H1e=None, **kwargs):
    ...
    print("!!!!!!!!! 2 transform results in slater!!!!!!!")
    print("E in slater", E)
    print("rho emb shape", rhoEmb.shape)
    if E is not None:
        ...
        print("!!!ImpHam.H0", ImpHam.H0)

        E2 = E - np.sum(ImpHam.H1["cd"] * rhoEmb) * (2.0 / spin) - ImpHam.H0
        print(ImpHam.H1["cd"] * rhoEmb)
        ...
        Efrag = None
    print("!!!!!!!!slater Efrag", Efrag)
    return rhoImp, Efrag, nelec

def get_H_dmet(basis, lattice, ImpHam, last_dmu, imp_idx=None, dmu_idx=None, \
        add_vcor_to_E=False, vcor=None, compact=True, rebuild_JK=False, \
        rdm1_emb=None, rdm1_JK=None):
    """
    ...
    basis_k = lattice.R2k_basis(basis)
    hcore_k = lattice.getH1(kspace=True)
    print("hcore_k", hcore_k.shape)
    H1_scaled = transform_h1(hcore_k, basis_k)
    print("H1_scaled", H1_scaled.shape)
    # note the double counting from JK_core cf. HF energy
    print("!!!!!!!!!rebuild_JK", rebuild_JK)
    print("!!!!!!!!!rdm1_JK", rdm1_JK)
    print("!!!!!add_vcor_to_E", add_vcor_to_E)
    #quit()
    #rebuild_JK = False, rdm1_JK = False, add_vcor_to_E = none
    if rebuild_JK:
        if rdm1_JK is None:
            rdm1_JK = get_rho_glob_k(basis, lattice, rdm1_emb)
        rdm1_ao = make_basis.transform_rdm1_to_ao(rdm1_JK, lattice.C_ao_lo)
        veff = lattice.kmf.get_veff(dm_kpts=rdm1_ao)
        veff = make_basis.transform_h1_to_lo(veff, lattice.C_ao_lo)
        JK_core = transform_h1(veff, basis_k)
        veff_loc = get_veff(rdm1_emb, ImpHam.H2["ccdd"], hyb=1.0)
        JK_core -= veff_loc
        if spin == 1:
            JK_core *= 2.0
        if lattice.JK_core is not None:
    ...
    np.save('1hHalfcorePreScale', H1_scaled)
    H1_scaled = get_H1_scaled(H1_scaled, imp_idx, env_idx)
    ...
    #ERR
    print( ImpHam.H2["ccdd"][s].shape)
    print(nbasis)
    H2_scaled = get_H2_scaled(H2_scaled, imp_idx, env_idx)


    ImpHam_dmet = integral.Integral(nbasis, spin == 1, False, lattice.getH0(),\
...
def get_active_projector(act_idx, rdm1, ovlp, orth=True, tol=1e-9):
    ...
    P_non_singular = []
    nocc = np.zeros((spin, nkpts), dtype=np.int)
    for s in range(spin):
        P = []
