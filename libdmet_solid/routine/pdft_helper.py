...
Including KRKS_LO, get_veff_lo_rks, KUKS_LO, get_veff_lo_uks.
...
from libdmet_solid.routine.krkspu import KRKSpU
from libdmet_solid.routine.kukspu import KUKSpU
...
class KRKS_LO(krks.KRKS):
    '''RKS class adapted for PBCs with k-point sampling, in LO basis.
    '''
    def __init__(self, cell, kpts=np.zeros((1,3)), xc='LDA,VWN', C_ao_lo=None, \
            dm_core_ao=None):
        ...
        if dm_core_ao is not None:
            ...
            self.nelec_core = np.einsum('kpq, kqp ->', self.dm_core_ao, ovlp).real \
                    / len(kpts)
        else:
...
class KUKS_LO(kuks.KUKS):
    '''UKS class adapted for PBCs with k-point sampling, in LO basis.
    '''
    def __init__(self, cell, kpts=np.zeros((1,3)), xc='LDA,VWN', \
        ...
        if dm_core_ao is not None:
            self.dm_core_ao = np.asarray(dm_core_ao)
            ovlp = np.asarray(self.get_ovlp())
            self.nelec_core = np.einsum('skpq, kqp ->', self.dm_core_ao, ovlp).real \
                    / len(kpts)
        else:
            self.dm_core_ao = None
            self.nelec_core = 0
