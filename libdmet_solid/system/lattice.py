...
import libdmet_solid.system.hamiltonian as ham
from  libdmet_solid.utils import logger as log
from libdmet_solid.utils.misc import add_spin_dim, max_abs, format_idx, Iterable

from libdmet_solid.system.fourier import *
from libdmet_solid.system.analyze import *
from libdmet_solid.utils.iotools  import *
...
class Lattice(object):
    def __init__(self, cell, kmesh):
        ...
        self.kmesh = kmesh
        self.nscsites = self.nao = self.supercell.nsites = self.cell.nao_nr()
    ...
    def FFTtoT(self, B):
        return FFTtoT(B, self.kmesh)

    def k2R(self, A):
        return k2R(A, self.kmesh)
    
    def R2k(self, B):
        return R2k(B, self.kmesh)
    
    def k2R_H2(self, H2_k):
        return k2R_H2(H2_k, self.phase_k2R)
    ...
    def set_Ham(self, kmf, df, C_ao_lo, eri_symmetry=1, \
        ...
        if isinstance(self.kmf, pdft.rks.KohnShamDFT): # DFT
            ...
            veff = vhf = pbc_hp.get_veff(kmf, vj, vk, vxc=None)
        # fock, fock_hf
        if fock is None:
            fock = hcore + veff
        fock_hf = hcore + vhf
    ...
    def update_Ham(self, rdm1_lo_R, rdm1_lo_k=None):
        """
        ...
        self.set_Ham(self.kmf, self.df, self.C_ao_lo, self.eri_symmetry, \
            ...
            fock=None, vj=None, vk=None, vxc=None, \
...
class LatticeModel(Lattice):
    ...
    def update_Ham(self, rdm1_lo_R, rdm1_lo_k=None, fock_lo_k=None):
        """
        Update fock matrix based on the new rdm from DMET,
        essentially follow Knizia JCTC 2013
        """
