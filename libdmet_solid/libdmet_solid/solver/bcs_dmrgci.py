...
from libdmet_solid.routine.localizer import Localizer
from libdmet_solid.utils.munkres import Munkres, make_cost_matrix
from libdmet_solid.routine.bcs_helper import extractRdm, basisToCanonical, basisToSpin
from libdmet_solid.integral.integral_emb_casci import transform
from libdmet_solid.integral.integral_emb_casci_save_mem import transform as transform_save_mem
from libdmet_solid.integral.integral_localize import transform as transform_local
from libdmet_solid.integral.integral_localize_qp import transform as transform_local_qp
from libdmet_solid.solver.dmrgci import gaopt, momopt
...
def get_BCS_mo(scfsolver, Ham, guess, max_memory=40000, Mu=None):
    ...
    if Mu is None:
        Mu = 0.0
    E_HFB, GRho_HFB = scfsolver.HFB(Mu=Mu, tol=1e-10, MaxIter=150, \
            InitGuess=guess)
...
def get_qps_local(ncas, nPAO, PAOidx, mo, mo_energy, nImp, ImpHam):
    ...
    ordA, ordB = np.argsort(wA, kind='mergesort'), \
            np.argsort(wB, kind='mergesort')
    log.debug(1, "Spin A:\nCoulomb repulsion: \n%s\nImpurity weight: \n%s", \
            fA[ordA], wA[ordA])
    log.debug(1, "Spin B:\nCoulomb repulsion: \n%s\nImpurity weight: \n%s", \
            fB[ordB], wB[ordB])
...
def get_qps_energy(ncas, mo, mo_energy):
    ...
    for idx in order[:ncas]: # beta
        if cas_energy[idx] < -1e-4:
            cas[1]['v'].append(cas_temp[list(range(norb, norb*2)) + \
                    list(range(norb)), idx])
        elif cas_energy[idx] > 1e-4:
            cas[1]['o'].append(cas_temp[list(range(norb, norb*2)) + \
                    list(range(norb)), idx])
        else:
            cas[1]['p'].append(cas_temp[list(range(norb, norb*2)) + \
                    list(range(norb)), idx])
    casinfo = [(len(cas[i]['o']), len(cas[i]['p']), len(cas[i]['v'])) \
               for i in range(2)]
    cas = np.asarray([(np.asarray(cas[i]['o'] + cas[i]['p'] + cas[i]['v']).T) \
            for i in range(2)])
...
class BCSDmrgCI(object):
    ...
    def run(self, Ham, ci_args={}, guess=None, basis=None, similar=False, \
        ...
        if basis is not None:
            ...
            nA_HFB, nB_HFB = np.sum(np.diag(GRhoHFB)[:nImp]), \
                    np.sum(1. - np.diag(GRhoHFB)[nbasis:nbasis+nImp])
