...
def test_half_imp():
    ...
    with pytest.warns(UserWarning):
        ...
        from libdmet_solid.dmet import Hubbard
        ...
        ImpHam, _ = slater.get_emb_Ham(Lat, basis, vcor, eri_symmetry=symm, \
                int_bath=int_bath)
        # RHF solver
        from libdmet_solid.solver.scf import SCF
        ...
        from libdmet_solid.routine.slater_helper import get_rho_glob_R
        rho_glob = get_rho_glob_R(basis[0], Lat, rdm1_emb, symmetric=False)
        print ("rho_glob diff")
        print (np.max(np.abs(rho_glob - Lat.rdm1_lo_R)))

        from libdmet_solid.routine.slater import get_H_dmet
        ...
        from libdmet_solid.routine.slater import get_E_dmet_HF
        last_dmu = 0.0
        ...
        from libdmet_solid.solver.impurity_solver import FCI, FCI_AO
        from libdmet_solid.dmet.Hubbard import transformResults, SolveImpHam_with_fitting, apply_dmu
        ImpHam, _ = slater.get_emb_Ham(Lat, basis, vcor, eri_symmetry=symm,
                int_bath=int_bath)
