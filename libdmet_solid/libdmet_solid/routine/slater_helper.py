...
from libdmet_solid.utils.misc import (mdot, max_abs, find, add_spin_dim, \
        Iterable, format_idx)
...
def get_rho_glob_R(basis, lattice, rho_emb, symmetric=True, compact=True):
    ...
    for basis_I, lattice_I, rho_emb_I in zip(basis_col, lattice_col, rho_emb_col):
        log.debug(0, "Build rdm1_glob, impurity %s, indices: %s", I_idx, format_idx(lattice_I.imp_idx))
        ...
        if compact:
            ...
            for R in range(ncells):
                ...
                print("imp_idx_0", imp_idx_0)
                env_idx_0 = np.where(~tmp)[0]
                print("env_idx_0", env_idx_0)
        ...
        rho_glob += rho_R
        I_idx += 1
        print(rho_glob.shape)
    rho = lattice.expand(rho_glob)
    print(rho.shape)
    np.save('rhoGlobZ', rho)
    return rho_glob

def get_rho_glob_k(basis, lattice, rho_emb, symmetric=True, compact=True):
    rho_R = get_rho_glob_R(basis, lattice, rho_emb, symmetric=symmetric, compact=compact)
...
def get_rho_glob_full(basis, lattice, rho_emb, symmetric=True, compact=True):
    ...
    Use democratic partitioning.
    """
    rho_glob_R = get_rho_glob_R(basis, lattice, rho_emb, symmetric=symmetric, compact=compact)
    if compact:
        if isinstance(lattice, Iterable):
    ...
    for J in range(ncells):
        for K in range(ncells):
            for L in range(ncells):
                ...
                rdm2_R[I_J, I_K, I_L] += lib.einsum('pqrs,ip,jq,kr,ls->ijkl', rdm2_emb,
                          basis_a[I_I], basis_a[I_J].conj(),
                          basis_b[I_K], basis_b[I_L].conj())
                rdm2_R[I_J, I_K, I_L] += lib.einsum('pqrs,ip,jq,kr,ls->ijkl', rdm2_emb,
                          basis_a[J_I], basis_a[J_J].conj(),
                          basis_b[J_K], basis_b[J_L].conj())
                rdm2_R[I_J, I_K, I_L] += lib.einsum('pqrs,ip,jq,kr,ls->ijkl', rdm2_emb,
                          basis_a[K_I], basis_a[K_J].conj(),
                          basis_b[K_K], basis_b[K_L].conj())
                rdm2_R[I_J, I_K, I_L] += lib.einsum('pqrs,ip,jq,kr,ls->ijkl', rdm2_emb,
                          basis_a[L_I], basis_b[L_J].conj(),
                          basis_b[L_K], basis_b[L_L].conj())
    return rdm2_R
...
def get_rho_pdmet_R(basis, lattice, phi_k):
    ...
    nscsites = lattice.nscsites
    rho_pdmet_k = np.zeros((nkpts, nscsites, nscsites), dtype=np.complex)
    for k in range(nkpts):
        phi = phi_k[k]
...
def unit2emb(H2_unit, neo):
    ...
    if H2_unit.ndim == 5:   # 1-fold
        ...
        raise ValueError
    fill_idx = tuple(map(slice, H2_unit[0].shape))
    for s in range(spin_pair):
        H2_emb[s][fill_idx] = H2_unit[s]
    return H2_emb
