...
from libdmet_solid.utils.misc import mdot, kdot, Iterable
from libdmet_solid.lo.lowdin import orth_cano, vec_lowdin, vec_lowdin_k, \
...
def get_iao_virt_max_ovlp(cell, C_ao_iao, S, minao=MINAO, full_virt=False, pmol=None): 
    ...
    """
    from libdmet_solid.basis_transform import find_closest_mo
    C_ao_iao = np.asarray(C_ao_iao)
    S = np.asarray(S)
    if pmol is None:
        pmol = reference_mol(cell, minao)
...
def get_idx_each(cell, minao=MINAO, full_virt=False, labels=None, \
        B2_labels=None, core_labels=None, kind='atom'):
    """
    ...
    if kind.lower() == 'id atom':
        symbols = [" ".join(label.split()[:2]) for label in labels]
    elif kind.lower() == 'atom':
        symbols = [label.split()[1] for label in labels]
    elif kind.lower() == 'atom nl':
        symbols = []
        for label in labels:
            ele = label.split()
            symbols.append(ele[1] + " " + ele[2][:2])
    elif kind.lower() == 'atom nl lz' or kind.lower() == 'atom nlm':
        symbols = [" ".join(label.split()[1:]) for label in labels]
    elif kind.lower() == 'id atom nl':
        symbols = []
        for label in labels:
            ele = label.split()
            symbols.append(ele[0] + " " + ele[1] + " " + ele[2][:2])
    elif kind.lower() == 'all':
        symbols = labels
    else:
        raise ValueError
...
def get_idx_each_atom(cell, minao=MINAO, full_virt=False, B2_labels=None, \
        core_labels=None, kind='atom'):
    return get_idx_each(cell, minao=minao, full_virt=full_virt, \
            B2_labels=B2_labels, core_labels=core_labels, kind=kind)

def get_idx_each_orbital(cell, minao=MINAO, full_virt=False, B2_labels=None, \
