...
def tril_take_idx(idx_list1, idx_list2=None, compact=False):
    ...
    if idx_list2 is None:
        idx_list2 = idx_list1
    if compact:
        idx = tril_idx(*tuple(zip(*reversed(\
                tuple(it.combinations_with_replacement(reversed(idx_list1), 2))))))
    else:
        idx = tril_idx(*tuple(zip(*it.product(idx_list1, idx_list2))))
    return idx
