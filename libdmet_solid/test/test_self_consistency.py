...
def test_self_consistency(orth):
    """
    ...
    for iter in range(MaxIter):
        ...
        ImpHam, H1e, basis = dmet.ConstructImpHam(Lat, rho, vcor, \
                matching=True, int_bath=int_bath, orth=orth)
    ...
    if orth:
        ...
        assert abs(EnergyImp - -1.244083473465) < 1e-4
        ### ************************************************************
        ### compare with KCCSD
        ### ************************************************************
        mycc = cc.KCCSD(kmf)
...
if __name__ == "__main__":
    test_self_consistency(orth=True)
