...
def test_cc_solver():
    ...
    C_ao_lo = C_ao_iao
    Lat.set_Ham(kmf, gdf, C_ao_lo, eri_symmetry=8)
    ...
    for iter in range(MaxIter):
        ...
        ImpHam, H1e, basis = dmet.ConstructImpHam(Lat, rho, vcor, matching=True, int_bath=int_bath, \
                add_vcor=False, t_reversal_symm=False, orth=True)
    ...
    assert abs(EnergyImp*nscsites/float(ncell_sc) - -1.242988933742) < 1e-4
