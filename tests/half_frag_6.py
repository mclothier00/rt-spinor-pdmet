...
if Ndots == 1:
    ...
    hubb_indx = np.array([2]) # impurity index
    ...
    h_site_r, V_site_r = make_hams.make_ham_multi_imp_anderson_realspace(
        Ndots, NL, NR, Vg, U_t0, timp, t_implead, Vbias, hubb_indx, tleads,
        boundary, Full_static)

# run static calculation
static = static_driver.static_pdmet(
    Nsites, Nele, Nfrag, impindx, h_site_r,
    V_site_r, U_t0, Maxitr, mf1RDM, tol,
...
if gen == True:
    system = transition_driver.rtog_transition(
        ...
        h_site_r, V_site_r, hamtype, hubb_indx, boundary)
else:
...
if Ndots == 1:
    # NOTE: must be changed manually; hardcoded based on number of sites
    if gen == True:
        hubb_indx = np.array([2])  # call the function I made for this!!
    else:
        hubb_indx = np.array([2])
    t_implead = 1.0
    tleads = 1.0
    ...
    h_site_r, V_site_r = make_hams.make_ham_multi_imp_anderson_realspace(
        Ndots, NL, NR, Vg, U_dyn, timp, t_implead, Vbias, hubb_indx, tleads, boundary, Full_dyn)


# run dynamics

if gen == True:
    # NOTE: done twice because currently above if/else statement is created for restricted hamiltonian; CREATE NEW CALL
    ...
    dynamics = dynamic_driver.dynamics_driver(
        h_site_r, V_site_r, hamtype, system, delt, dG, dX, U_dyn, A_nott, t_nott, omega,
        t_d, nproc, Nstep, Nprint, integrator, hubb_indx, laser_sites, init_time,
        laser_dyn)
