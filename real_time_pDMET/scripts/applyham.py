...
def apply_ham_pyscf_check(
    ...
    if not gen:
        if np.iscomplexobj(hmat) and np.iscomplexobj(Vmat):
            ...
            print("applyham.apply_ham_pyscf_check")
...
def contract_2e_complex(g2e, fcivec, norb, nelec, link_index=None):
    ...
    t1 = np.dot(g2e.reshape(norb * norb, -1), t1.reshape(norb * norb, -1))
    t1 = t1.reshape(norb, norb, na, nb)

    # data type of ci1 is now complex
    ci1 = np.zeros_like(fcivec, dtype=complex)
    for str0, tab in enumerate(link_indexa):
        for a, i, str1, sign in tab:
...
# NOTE: Forte2-dependent functions
