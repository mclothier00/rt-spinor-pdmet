...
def FCI_GS(h, V, Ecore, Norbs, Nele, gen=False, forte=False):
    # Subroutine to perform groundstate FCI calculation; uses pyscf by default
    # NOTE: PySCF follows chemists notation; Forte2 follows physicists notation!
    ...
    if forte:
        ...
        state = State(nel=Nele, multiplicity=1, ms=0.0)
        ci = _CIBase(
            mo_space=mo_space,
            state=state,
            ints=ints,
...
def get_corr1RDM(CIcoeffs, Norbs, Nele, gen=False, forte=False):
    # Subroutine to get the FCI 1RDM

    if forte:
        # NOTE: concerns: see get_corr12RDM concerns
        corr1RDM = _CIBase.make_sd_1rdm(CIcoeffs)
        print("not yet functional; use pyscf version")
        exit()
...
def get_corr12RDM(CIcoeffs, Norbs, Nele, gen=False, forte=False, ci=None):
    ...
    if forte:
        ...
        if gen:
            ...
            corr1RDM, corr2RDM = pyscf.fci.fci_dhf_slow.make_rdm12(
                CIcoeffs, Norbs, Nele
            )
