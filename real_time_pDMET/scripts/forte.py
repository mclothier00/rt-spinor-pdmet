...
class forte:
    ...
    def __init__(self, Norbs, Nele, gen):
        self.Norbs = Norbs
        self.Nele = Nele
        self.gen = gen
    ...
    def FCI_GS_forte(self, h, V, Ecore):
        ...
        ints.H = h
        ints.V = V_forte
        mo_space = MOSpace(nmo=self.Norbs, active_orbitals=list(range(self.Norbs)))
        ...
        self.ci = _CIBase(
            ...
            active_orbsym=[[0] * self.Norbs],  # C1 symmetry
            log_level=0,
            maxiter=200,
    ...
    def get_corr1RDM(self, CIcoeffs):
        ...
        if self.gen:
            ...
            corr1RDM = sigma_builder.sf_1rdm(
                CIcoeffs[:, 0].copy(), CIcoeffs[:, 0].copy()
            )
        return corr1RDM

    #####################################################################

    def get_corr12RDM(self, CIcoeffs):
        ...
        if self.gen:
            ...
            corr2RDM = sigma_builder.sf_2rdm(
                CIcoeffs[:, 0].copy(), CIcoeffs[:, 0].copy()
            )
