...
from libdmet_solid.solver.mp import UIMP2, GGMP2
from libdmet_solid.utils.misc import mdot, add_spin_dim, max_abs
from libdmet_solid.routine.bcs_helper import extractRdm, extractH1
...
# ZHC FIXME whether to restore? 8 or 4?
# ZHC unify the UHF routine using lambda function.
# *********************************************************************
...
class GGHF(scf.ghf.GHF):
    ...
    def get_jk(self, mol=None, dm=None, hermi=0, \
        ...
        if dm is None: dm = self.make_rdm1()
        dm = np.asarray(dm)
        log.eassert(self._eri is not None, \
                "mf._eri should be intialized.")
        vj, vk = _get_jk(dm, self._eri)
        return vj[0], vk[0]
...
def _get_jk_ghf(dm, eri):
    ...
    # vj01, vj10, vk22
    eri_ab = ao2mo.restore(4, eri[2], nao)
    vj01, _ = scf.hf.dot_eri_dm(eri_ab, dm_bb, hermi=1, \
            with_j=True, with_k=False)
    vj10, _ = scf.hf.dot_eri_dm(eri_ab.T, dm_aa, hermi=1, \
            with_j=True, with_k=False)
    # ZHC NOTE FIXME this hermi should be 0 ?
    _, vk22 = scf.hf.dot_eri_dm(eri_ab, dm_ab, hermi=0, \
...
class GIHF(GGHF):
    ...
    def energy_elec(self, dm=None, h1e=None, vhf=None):
        ...
        h1e_no_mu = h1e.copy()
        if self.Mu is not None:
            h1e_no_mu[:norb, :norb] += np.eye(norb) * self.Mu
            h1e_no_mu[norb:, norb:] -= np.eye(norb) * self.Mu
...
class SCF(object):
    ...
    def HF(self, DiisDim=12, MaxIter=50, InitGuess=None, tol=1e-6, \
        ...
        if not self.spinRestricted: # UHF
            ...
            self.mf.h1e = self.integral.H1["cd"].copy()
    ...
    def GHF(self, Mu, DiisDim=12, MaxIter=50, InitGuess=None, \
            tol=1e-6, do_diis=True):
        log.eassert(self.sys_initialized and self.integral_initialized, \
        ...
        if self.mol.nelectron != norb:
            log.warn("GHF nelectron (%s) != nao (%s) !", \
                    self.mol.nelectron, norb)
    ...
    def GGHF(self, Mu=0.0, DiisDim=12, MaxIter=50, InitGuess=None, \
            tol=1e-6, do_diis=True):
        log.eassert(self.sys_initialized and self.integral_initialized, \
                "components for GHF calculation are not ready"
                "\nsys_init = %s\nint_init = %s", \
        ...
        if self.mol.nelectron * 2 != norb:
            log.warn("GHF nelectron (%s) * 2 != nso (%s) !", \
                    self.mol.nelectron, norb)
        nelec = self.mol.nelectron
        
        log.result("Generalized Hartree-Fock with pyscf")
        self.mf = GGHF(self.mol)
        self.mf.diis_space = DiisDim
        self.mf.max_cycle = MaxIter
        self.mf.conv_tol = tol
    ...
    def GMP2(self, mo_energy=None, mo_coeff=None, mo_occ=None, frozen=None):
        ...
        self.mp = GGMP2(self.mf, frozen=frozen, mo_coeff=mo_coeff, \
                mo_occ=mo_occ)
        E, t2 = self.mp.kernel(mo_energy=mo_energy, mo_coeff=mo_coeff)
        rdm1 = self.mp.make_rdm1(ao_repr=True)
        return E, rdm1
...
#def rhf(integral, nelec=None, tol=1e-12, dm=None):
#    """
#    RHF with customized integrals. Just for convenience.
#    """
#    mol = gto.M()
#    norb = integral.norb
#    if nelec is None:
...
#    mf.conv_tol = tol
#
#    mf.energy_nuc = lambda *args: integral.H0
#    mf.get_hcore = lambda *args: integral.H1["cd"][0]
#    mf.get_ovlp = lambda *args: integral.ovlp
#    mf._eri = ao2mo.restore(4, integral.H2["ccdd"][0], norb)
#    mf.kernel(dm=dm)
#    return mf
