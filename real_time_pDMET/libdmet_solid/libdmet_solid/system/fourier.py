...
from libdmet_solid.utils.misc import max_abs, format_idx, get_cart_prod_idx
...
def make_kpts_scaled(kmesh):
    ...
    """
    ks_each_axis = [np.fft.fftfreq(kmesh[d], 1.) for d in range(len(kmesh))]
...
def FFTtoK(A, kmesh):
    ...
    """
    return np.fft.fftn(A.reshape(tuple(kmesh) + A.shape[-2:]), \
        axes=range(len(kmesh))).reshape(A.shape)

def FFTtoT(B, kmesh):
    """
    from k space to real space.
    """
    A = np.fft.ifftn(B.reshape(tuple(kmesh) + B.shape[-2:]), \
        axes=range(len(kmesh))).reshape(B.shape)
    if max_abs(A.imag) < IMAG_DISCARD_TOL:
        A = A.real
    return A
...
def k2R_H2(H2, phase_k2R):
    ...
    if max_abs(H2_R.imag) < IMAG_DISCARD_TOL:
        H2_R = H2_R.real
    return H2_R
...
def find_idx_R_vec(R_vec, skmesh, scell, tol=1e-10):
    ...
    cart_prod = (R_vec_scaled + tol).astype(np.int)
...
def fold_h1(hcore_0_k, latt_0, latt_1, resort_row=False, resort_col=False, \
    ...
    kmesh_0 = latt_0.kmesh
    assert np.prod(kmesh_0) == nkpts_0
