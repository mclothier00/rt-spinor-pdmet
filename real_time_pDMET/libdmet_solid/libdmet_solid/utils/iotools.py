...
def build_unitcell(cellsize, atoms, frac=False, center=None):
    import libdmet_solid.system.lattice as lat
...
def sc2POSCAR(sc, fout, elements=None, vac=15.0):
    ...
    fwrite.write("  generated using libdmet_solid.utils.iotools\n")
    fwrite.write("%10.6f\n" % 1)
...
def sc2XYZ(sc, fout, elements=None):
    ...
    fout.write("%d\n" % sc.nsites)
    fout.write("generated using libdmet_solid.utils.iotools\n")
...
def read_poscar(fname="POSCAR"):
    ...
    from libdmet_solid.system.lattice import Frac2Real
    with open(fname, 'r') as f:
        ...
        if all([name.isdigit() for name in sp_names]):
            ...
            sp_nums = np.fromstring(lines[5], dtype=np.int, sep=' ')
            sp_names = ["X" for i in range(len(sp_nums))]
            line_no = 6 
        else:
            sp_nums = np.fromstring(lines[6], dtype=np.int, sep=' ')
            line_no = 7
...
def write_poscar(cell, fname="POSCAR", species=True, cart=True):
    """
    ...
    """
    from libdmet_solid.system.lattice import Real2Frac
    if not cell._built:
    ...
    with open(fname, 'w') as f:
        f.write("POSCAR generated from libdmet_solid \n")
        f.write("1.0 \n")
...
def change_cell_shape(cell, vec, origin=np.zeros(3), \
        search_range=[2, 2, 2], tol=1e-6):
    """
