...
from libdmet_solid.utils.misc import mdot, max_abs, add_spin_dim, Iterable
from libdmet_solid.routine import ftsystem
from libdmet_solid.settings import IMAG_DISCARD_TOL 
from libdmet_solid.routine.pbc_helper import *
...
def HFB(lattice, vcor, restricted, mu=0.0, beta=np.inf, fix_mu=False, \
    ...
    from libdmet_solid.routine.bcs_helper import extractRdm
...
def assignocc(ew, nelec, beta, mu0, fix_mu=False, thr_deg=1e-6, Sz=None, \
    ...
    if (Sz is None) and (not isinstance(nelec, Iterable)):
        ...
        if beta < np.inf:
            ...
            nelec = check_nelec(nelec, None)[0]   
            if np.sum(ew < mu0-thr_deg) <= nelec and \
