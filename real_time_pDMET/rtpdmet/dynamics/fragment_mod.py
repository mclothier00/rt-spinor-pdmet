...
class fragment:
    ...
    def get_Hemb(self, h_site, V_site, hamtype=0, hubsite_indx=None, gen=False):
        ...
        if not gen:
            ...
            if hamtype == 0:
                ...
                V_emb = 0.5 * (
                    V_emb + V_emb.transpose(2, 3, 0, 1)
                )  # sym2, (ij|kl) = (kl|ij)
                V_emb = 0.5 * (
                    V_emb + V_emb.conj().transpose(1, 0, 3, 2)
                )  # sym1, (ij|kl) = (ji|lk)*
                ...
                self.V_emb = 0.5 * (
                    self.V_emb + self.V_emb.transpose(2, 3, 0, 1)
                )  # sym2, (ij|kl) = (kl|ij)
                self.V_emb = 0.5 * (
                    self.V_emb + self.V_emb.conj().transpose(1, 0, 3, 2)
                )  # sym1, (ij|kl) = (ji|lk)*
        ...
        if gen:
            ...
            if hamtype == 0:
                ...
                for s in range(0, len(hubsite_indx), 2):
                    ...
                    # (iα iα | iβ iβ) term
                    self.V_emb += V_site * np.einsum(
                        "i,j,k,l->ijkl", ra.conj(), ra, rb.conj(), rb
                    )
                    # (iβ iβ | iα iα) term
                    self.V_emb += V_site * np.einsum(
                        "i,j,k,l->ijkl", rb.conj(), rb, ra.conj(), ra
                    )
            ...
            if hamtype == 0:
                ...
                for s in range(0, len(hubsite_indx), 2):
                    ...
                    core_int += (
                        V_site
                        * np.einsum("i,j->ij", ra_imp.conj(), ra_imp)
                        * np.dot(rb_core.conj(), rb_core)
                    )
                    core_int += (
                        V_site
                        * np.einsum("i,j->ij", rb_imp.conj(), rb_imp)
                        * np.dot(ra_core.conj(), ra_core)
                    )
                    # Exchange: sum_n V[i,n,n,j]
                    core_int -= (
                        V_site
                        * np.einsum("i,j->ij", ra_imp.conj(), rb_imp)
                        * np.dot(ra_core, rb_core.conj())
                    )
                    core_int -= (
                        V_site
                        * np.einsum("i,j->ij", rb_imp.conj(), ra_imp)
                        * np.dot(rb_core, ra_core.conj())
                    )
    ...
    def get_corr12RDM(self):
        ...
        # NOTE: signs are different between CI coeffs; so far does not seem to impact corr1RDM
        ...
        if gen:
            ...
            if hamtype == 1:
                ...
                for s in range(0, len(hubsite_indx), 2):
                    ...
                    ra = self.rotmat[ia, :]
                    rb = self.rotmat[ib, :]
                    ra_core = self.rotmat[ia, self.corerange]
                    rb_core = self.rotmat[ib, self.corerange]
                    # Coulomb: sum_k V[i,j,k,k]
                    IFmat += (
                        V_site
                        * np.einsum("i,j->ij", ra.conj(), ra)
                        * np.dot(rb_core.conj(), rb_core)
                    )
                    IFmat += (
                        V_site
                        * np.einsum("i,j->ij", rb.conj(), rb)
                        * np.dot(ra_core.conj(), ra_core)
                    )
                    # Exchange: sum_k V[i,k,k,j]
                    IFmat -= (
                        V_site
                        * np.einsum("i,j->ij", ra.conj(), rb)
                        * np.dot(ra_core, rb_core.conj())
                    )
                    IFmat -= (
                        V_site
                        * np.einsum("i,j->ij", rb.conj(), ra)
                        * np.dot(rb_core, ra_core.conj())
                    )
            ...
            if hamtype == 1:
                AFmat = np.zeros((self.Nsites, self.Nsites), dtype=complex)
                for s in range(0, len(hubsite_indx), 2):
                    ia, ib = hubsite_indx[s], hubsite_indx[s + 1]
                    ra = self.rotmat[ia, :]
                    rb = self.rotmat[ib, :]
                    ra_act = self.rotmat[ia, actrange]
                    rb_act = self.rotmat[ib, actrange]
                    # corr1RDM-weighted partner densities
                    Dra = np.dot(ra_act, self.corr1RDM)
                    Drb = np.dot(rb_act, self.corr1RDM)
                    # Coulomb: sum_kl D[k,l] V[i,j,l,k]
                    AFmat += (
                        V_site
                        * np.einsum("i,j->ij", ra.conj(), ra)
                        * np.dot(Drb, rb_act.conj())
                    )
                    AFmat += (
                        V_site
                        * np.einsum("i,j->ij", rb.conj(), rb)
                        * np.dot(Dra, ra_act.conj())
                    )
                    # Exchange: sum_kl D[k,l] V[i,l,k,j]
                    AFmat -= (
                        V_site
                        * np.einsum("i,j->ij", ra.conj(), rb)
                        * np.dot(Dra, rb_act.conj())
                    )
                    AFmat -= (
                        V_site
                        * np.einsum("i,j->ij", rb.conj(), ra)
                        * np.dot(Drb, ra_act.conj())
                    )
            ...
            if hamtype == 1:
                ...
                for s in range(0, len(hubsite_indx), 2):
                    ...
                    # (p, r) = (alpha, beta)
                    wab = np.einsum(
                        "jklm,k,l,m->j", self.corr2RDM, ra_act, rb_act.conj(), rb_act
                    )
                    # (p, r) = (beta, alpha)
                    wba = np.einsum(
                        "jklm,k,l,m->j", self.corr2RDM, rb_act, ra_act.conj(), ra_act
                    )
                    genFmat[actrange, :] += V_site * np.transpose(
                        np.einsum("i,j->ij", ra.conj(), wab)
                        + np.einsum("i,j->ij", rb.conj(), wba)
                    )
    ...
    def get_Xmat(self, mf1RDM, ddt_mf1RDM, dX):
